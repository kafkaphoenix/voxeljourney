...
struct Frustum {
    std::array<glm::vec4, 6> planes;  // x,y,z,w: plane normal.xyz, d
};

inline Frustum calculateFrustum(const glm::mat4& viewProj) {
    Frustum frustum{};
    // Left
    frustum.planes[0] = glm::vec4(viewProj[0][3] + viewProj[0][0], viewProj[1][3] + viewProj[1][0],
                                  viewProj[2][3] + viewProj[2][0], viewProj[3][3] + viewProj[3][0]);
    // Right
    frustum.planes[1] = glm::vec4(viewProj[0][3] - viewProj[0][0], viewProj[1][3] - viewProj[1][0],
                                  viewProj[2][3] - viewProj[2][0], viewProj[3][3] - viewProj[3][0]);
    // Bottom
    frustum.planes[2] = glm::vec4(viewProj[0][3] + viewProj[0][1], viewProj[1][3] + viewProj[1][1],
                                  viewProj[2][3] + viewProj[2][1], viewProj[3][3] + viewProj[3][1]);
    // Top
    frustum.planes[3] = glm::vec4(viewProj[0][3] - viewProj[0][1], viewProj[1][3] - viewProj[1][1],
                                  viewProj[2][3] - viewProj[2][1], viewProj[3][3] - viewProj[3][1]);
    // Near
    frustum.planes[4] = glm::vec4(viewProj[0][3] + viewProj[0][2], viewProj[1][3] + viewProj[1][2],
                                  viewProj[2][3] + viewProj[2][2], viewProj[3][3] + viewProj[3][2]);
    // Far
    frustum.planes[5] = glm::vec4(viewProj[0][3] - viewProj[0][2], viewProj[1][3] - viewProj[1][2],
                                  viewProj[2][3] - viewProj[2][2], viewProj[3][3] - viewProj[3][2]);
    // Normalize
...
inline bool frustumIntersectsAABB(const Frustum& frustum, const AABB& aabb, const glm::mat4& modelMatrix) {
    ...
    glm::vec3 wsMin(FLT_MAX);
    glm::vec3 wsMax(-FLT_MAX);
    for (int i = 0; i < 8; ++i) {
        glm::vec3 corner = glm::vec3(modelMatrix * glm::vec4(((i & 1) != 0 ? aabb.max.x : aabb.min.x),
                                                             ((i & 2) != 0 ? aabb.max.y : aabb.min.y),
                                                             ((i & 4) != 0 ? aabb.max.z : aabb.min.z), 1.0f));
